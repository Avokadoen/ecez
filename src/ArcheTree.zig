...
const Node = struct {
    children_indices: std.ArrayList(usize),
    type_hash: u64,
    archetype: *anyopaque,
};
...
pub fn init(allocator: Allocator) !ArcheTree {
    ...
    var root_archetype = allocator.create(Root);
    root_archetype.* = Root.init(allocator);
    errdefer root_archetype.deinit();
    const root_node = Node{
        .children_indices = std.ArrayList(usize).init(allocator),
        .type_hash = 0,
        .archetype = @ptrCast(*anyopaque, root_archetype),
    };
...
pub fn deinit(self: ArcheTree) void {
    ...
    for (self.node_storage.items) |*node| {
        if (node.archetype) |*opaquearch| {
            const arch = @ptrCast(*archetype.FromTypesArray(node.component_types), opaquearch);
            arch.deinit();
        }
    }
